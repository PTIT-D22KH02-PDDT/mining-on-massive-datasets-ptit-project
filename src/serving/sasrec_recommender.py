...
try:
    ...
    import torch
    from torch import diag, logical_and, logical_or, nn, tile
    ...
    class SASRec(py_light.LightningModule):
        def __init__(
            ...
            **kwargs,
            ...
            self.future_mask = torch.triu(
                torch.ones(max_len, max_len) * float("-inf"), diagonal=1
            )
            self.register_buffer("future_mask_const", self.future_mask)
            self.register_buffer(
                "seq_diag_const", ~diag(torch.ones(max_len, dtype=torch.bool))
            )
            self.register_buffer("bias_ones", torch.ones([batch_size, max_len, 1]))

            if output_bias and share_embeddings:
                self.item_embedding = nn.Embedding(
                    num_items + 1, hidden_size + 1, padding_idx=0
                )
            else:
                self.item_embedding = nn.Embedding(
            ...
            self.positional_embedding_layer = DynamicPositionEmbedding(
                max_len, hidden_size
            )
            ...
            self.encoder = nn.TransformerEncoder(
                encoder_layer, num_layers=num_layers, norm=self.norm
            )
        ...
        def merge_attn_masks(self, padding_mask):
            ...
            future_masks = tile(
                self.future_mask_const[:seq_len, :seq_len], (batch_size, 1, 1)
            )
            merged_masks = logical_or(padding_mask_broadcast, future_masks)
            diag_masks = tile(
                self.seq_diag_const[:seq_len, :seq_len], (batch_size, 1, 1)
            )
        ...
        def forward(self, item_indices, mask):
            ...
            items = (
                self.item_embedding(item_indices)[:, :, :-1]
...
class SASRecRecommender:
    ...
    def __init__(
        self, checkpoint_path: str = None, remote_url: str = SASREC_REMOTE_URL
    ):
        ...
        if self.remote_url:
            ...
            if os.path.exists(self.checkpoint_path):
                try:
                    self.model = SASRec.load_from_checkpoint(
                        self.checkpoint_path, map_location=self.device
                    )
                    self.model.eval()
                    self.model.to(self.device)
                    logger.info(
                        f"SASRec initialized in LOCAL mode from {self.checkpoint_path}"
                    )
            ...
            logger.warning(
                "Torch not available and no Remote URL set. Using dummy mode."
            )
    ...
    def _predict_local(self, session_aids: List[int], top_k: int) -> List[int]:
        ...
        with torch.no_grad():
            ...
            logits = multiply_head_with_embedding(
                last_hidden, self.model.output_embedding.weight
            ).squeeze(0)
            logits[0] = float("-inf")
