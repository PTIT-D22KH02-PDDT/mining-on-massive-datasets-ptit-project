...
- v2: 2026-05-22 22:12 (post-tuning: circuit breaker + CPU limits + trigger + Kafka tune + merged aggregations)
...
| # | Thay đổi | Target |
|---|----------|--------|
| 1 | Circuit breaker: `fail_max=3→2`, SASRec timeout `10s→2s` | API p95 < 500ms |
| 2 | CPU limits: Spark `cpus=4→2`, Kafka `cpus=2`, API `cpus=2` | Giảm contention |
...
1. **SASRec timeout 2s** không đủ để cắt tail (vì circuit breaker mất 2 lần fail mới open → ít nhất 4s cho 2 request đầu).
2. **Spark CPU giảm** (4→2) khiến batch kéo dài → Kafka queue tích tụ → Redis buffer tăng → API latency bị ảnh hưởng gián tiếp.
3. **Kafka throughput giảm 30%** (xem mục 3) → event handler (`kafka_queue.put_nowait()`) chờ lâu hơn.

### Giải pháp

- **SASRec local model**: bỏ remote call hoàn toàn (local inference < 50ms), triệt tiêu tail latency từ circuit breaker.
- **Covisitation cache**: precompute top-K per session → API p95 < 500ms mà không cần SASRec.
...
|----------|----------|--------|
| Circuit breaker 2s timeout | API p95 giảm | p95 tăng (+10%) — không đủ, remote call vẫn là root cause |
...
2. **Circuit breaker tuning không đủ** để fix API tail latency. Cần local SASRec hoặc covisitation cache.
3. **Kafka tune (IO/NETWORK threads) phản tác dụng** với CPU hạn chế. Cần revert hoặc giảm.
4. **Merge aggregations + cache** có lợi nhưng không đáng kể (giảm 1 shuffle trong 6).
5. **E2E ổn định** — không bị ảnh hưởng bởi các thay đổi.
...
| 3 | **Covisitation cache**: top-K precompute | API p95 < 500ms | Không cần remote call |
| 4 | **Local SASRec model** | API p95 < 100ms | Triệt tiêu tail latency hoàn toàn |
| 5 | **Separate disk cho Kafka** | Kafka P95 < 1s | Giảm I/O contention |
