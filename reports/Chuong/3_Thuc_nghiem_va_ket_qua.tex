...
Spark Streaming đọc dữ liệu từ Kafka thông qua cơ chế readStream của Spark Structured Streaming. Mỗi lô nhỏ (micro-batch) được kích hoạt với khoảng kích hoạt (trigger interval) 10 giây và áp dụng cơ chế kiểm soát phản áp (backpressure) để giới hạn số bản ghi xử lý mỗi batch. Dữ liệu thô dạng JSON được phân tích cú pháp (parse) với lược đồ (schema) cố định gồm các trường:
\begin{itemize}
...
\begin{enumerate}
    \item[\textbf{A.}] \textbf{Thống kê theo khoảng thời gian (\textbf{stats\_hourly}):} Tính toán các chỉ số tổng hợp theo cửa sổ (window) 1 phút: tổng số sự kiện, số phiên duy nhất (sử dụng thuật toán đếm xấp xỉ với sai số $\approx$2\%), số sản phẩm duy nhất, số sự kiện theo từng loại (click, cart, order), và tỷ lệ chuyển đổi click qua cart, cart qua order. Dữ liệu được ghi vào bảng thống kê theo giờ theo chế độ thêm (append) (mỗi cửa sổ (window) là một dòng mới). Bảng này phục vụ dashboard theo dõi xu hướng theo thời gian và phát hiện bất thường về lưu lượng.
...
\section{Triển khai}
Toàn bộ hệ thống được đóng gói bằng Docker và Docker Compose, mỗi thành phần chạy trên một container riêng biệt và giao tiếp qua mạng nội bộ. Mỗi service có Dockerfile tùy chỉnh phù hợp với vai trò của nó - API service dùng image nhẹ không cần Java, trong khi Spark Streaming và các batch job cần image có cả Java và Python.

Hệ thống sử dụng ba file cấu hình Docker Compose: file chính định nghĩa toàn bộ service và cụm Spark, file bổ sung cho môi trường phát triển với hot-reload, và file cho triển khai production kéo image từ registry.
...
Cấu hình hệ thống được quản lý qua file cấu hình ứng dụng và file biến môi trường lưu thông tin dịch vụ (cơ sở dữ liệu, hàng đợi tin nhắn, URL máy chủ mô hình). Hệ thống có thể mở rộng bằng cách tăng số partition Kafka hoặc điều chỉnh tài nguyên Spark.
